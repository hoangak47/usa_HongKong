class Cart {
	...
	getDistrict(target) {
		...
		fetch(this.base_url + 'cart/get-district', {
			...
			.then((result) => {
				if (result.districts) {
					this.populateSelect(result.districts, selectDistrict,'Chọn quận huyện');
				}
				holdonClose();
			});
	}
		...
		fetch(this.base_url + 'cart/get-ward', {
			...
			.then((result) => {
				if (result.wards) {
					...
					this.populateSelect(result.wards, selectDistrict,'Chọn phường xã');
				}
				holdonClose();
			});
	}
	...
	addCart(target) {
		...
		if (target.getAttribute('data-q') > 0 ) {
			quantity = parseInt(target.getAttribute('data-q'), 10); // Chuyển đổi id sang số nguyên
		}else{
			quantity = parseInt(this.wrapper.querySelector('.qty-pro').value, 10);
		}
		holdonOpen();
		var form_data = new FormData();
		form_data.append('id', id);
	...
	populateSelect(optionsArray = [], selectElement = '',optionDefault = 'Chọn danh mục') {
		selectElement.innerHTML = '<option value="">' + optionDefault + '</option>';
