...
class ProductController
{
    ...
    public function deleteList($com, $act, $type, Request $request)
    {
        if (!empty($request->id)) {
            ...
            $row = ProductListModel::select('id', 'photo','icon')
                ->where('id', $id)
                ->first();
            ...
            if (!empty($row)) {
                ...
                if (File::exists(upload('product', $row['icon'], true))) {
                    File::delete(upload('product', $row['icon'], true));
                }
    ...
    public function deleteCat($com, $act, $type, Request $request)
    {
        ...
        if (!empty($request->id)) {
            $id = $request->id;
            $row = ProductCatModel::select('id', 'photo','icon')
                ->where('id', $id)
                ->first();
            $rowGallery = GalleryModel::select('id', 'photo')->where('id_parent', $id)->where('com', $com)->where('type', $type)->get();
            if (!empty($row)) {
    ...
    public function deleteItem($com, $act, $type, Request $request)
    {
        if (!empty($request->id)) {
            ...
            $row = ProductItemModel::select('id', 'photo','icon')
                ->where('id', $id)
                ->first();
            $rowGallery = GalleryModel::select('id', 'photo')->where('id_parent', $id)->where('com', $com)->where('type', $type)->get();
            if (!empty($row)) {
    ...
    public function deleteSub($com, $act, $type, Request $request)
    {
        if (!empty($request->id)) {
            ...
            $row = ProductSubModel::select('id', 'photo','icon')
                ->where('id', $id)
                ->first();
            $rowGallery = GalleryModel::select('id', 'photo')->where('id_parent', $id)->where('com', $com)->where('type', $type)->get();
            if (!empty($row)) {
    ...
    public function edit($com, $act, $type, Request $request)
    {
        ...
        if (!empty($this->configType->$type->gallery) && $item) {
            $gallery = $item->getPhotos('product')->where('type',$type)->get();
        }
        ...
        if (!empty($request->csrf_token)) {
            ...
            if ($id) {
                ...
                if (ProductModel::where('id', $id)->where('type', $type)->update($data)) {
                    ...
                    if (!empty($this->configType->$type->suggest)) {
                        $this->insertTags(ProductTagsModel::class, $request, $dataSuggest, $id, 'goi-y-hom-nay');
                    }
