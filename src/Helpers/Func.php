...
use NINA\Models\OrderStatusModel;
use NINA\Models\NewsModel;
use NINA\Models\SettingModel;
use NINA\Models\CounterModel;
use NINA\Models\PropertiesModel;
    ...
    public function allNoty()
    {
        ...
        if (!empty(config('type.newsletters'))) {
            foreach (config('type.newsletters') as $key => $v) {
                $allNoty += \NINA\Models\NewslettersModel::where('confirm_status', 1)->where('type', $key)->count();
            }
        }
        if (!empty(config('type.order'))) {
            $allNoty += \NINA\Models\OrdersModel::where('order_status', 1)->count();
        }
        if (!empty(config('type.comment'))) {
            $allNoty += \NINA\Models\CommentModel::where('status', '')->count();
        }
        return $allNoty;
    }
    /* Check URL */
...
*/
    ...
    public function nameImgCrop($string = '')
    {
        ...
        return $this->changeTitle($string[0]) . '-' . time() . '.' . end($string);
    }
    ...
    public function breadcrumbCategory($id=0){
        if(!empty($id)){
