...
class JsMinify
{
    ...
    {
        $this->debug = !(config('app.environment') == 'production');
        $this->access = array(
    ...
    public function init($name)
    {
        ...
        $cacheName = '';
        $cacheName = $name;
        $this->cacheFile = $this->cacheFile . $this->access['server'] . $this->access['folder'] . $cacheName . '.js';
        $this->cacheLink = $this->cacheLink . $this->access['asset'] . $this->access['folder'] . $cacheName . '.js';
    ...
    private function minify()
    {
        ...
        return '<script type="text/javascript" src="' . $this->cacheLink . ((config('app.environment')=='dev')?('?v='.filemtime($this->cacheFile)):'') . '"></script>';
    }
    ...
    private function links()
    {
        ...
        foreach ($this->path as $path) {
            ...
            $extension = strtolower($parts['extension']);
            if ($extension != 'js') die("Invalid file");
            $linkJs .= '<script type="text/javascript" src="' . $path['asset'] . '?v=' . $this->stringRandom(10) . '"></script>' . PHP_EOL;
        }
